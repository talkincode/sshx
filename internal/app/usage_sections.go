...
// Help text for the sshx command surface.
//
// Each block is defined exactly once: `sshx --help` prints every block in
// order, and `sshx <verb> --help` prints that verb's blocks plus the shared
// option blocks. Add or change text here, never in a second place.
...
Usage:
  ...
  sshx <verb> --help                              # Per-verb usage (add --json for sshx.help.v1)
`

const usageSSHOptions = `
SSH Options:
  ...
  --quiet, --no-notices    Suppress human notices on stderr (deprecation warnings,
                           narration). stdout, the exit code, and the JSON
...
Agent / Scripting Mode:
  ...
  Under --json, stdout carries the machine document and only the machine
  document; sshx never writes human text there, including on failure. Human
  notices (deprecation warnings, narration) go to stderr. A caller that merges
  the streams (2>&1) should pass --quiet, which suppresses the notices so the
  merged stream is still parseable; failures keep their JSON error_kind/error.
...
Guarded SQL Execution:
  ...
  open, or pass --statement-file=PATH.
  A statement that opens with a comment ("-- header") cannot be an option
  token, so it is taken as statement text; only option-shaped typos are
  rejected, and those name the intended option.
  ...
  --statement-file=PATH     Read the statement from a local file (mutually
                            exclusive with a positional statement)
...
const usagePlugin = `
Plugin Management:
  sshx plugin install <dir> [--replace] [--trust] [--json]
  ...
  plugin install is the audited way to provision an existing plugin directory:
  it stages the source with sshx's own modes, validates it through the same
  loader the executor uses, publishes it only if it is valid, and with --trust
  records the published digest in one step. Symlinks and non-regular entries are
  refused, and the copy is bounded (8MiB, 128 files). --replace preserves the
  previous plugin under ~/.sshx/plugin-backups.

  plugin list groups built-in capabilities and local plugins and always names
  the local root, so "no local plugins installed" is visible as such; a local
  entry reports builtin/trusted/valid and its digest. A missing plugin names the
...
const usageAuditQuery = `
Audit Query and Export:
  sshx audit query [filters] [--json]
  sshx audit export --to=<file> [filters]

  Read-only consumption of the local JSONL trail. Query never connects and never
  writes; export writes only the requested file.

  --since=YYYY-MM-DD        Inclusive lower bound (date or RFC3339)
  --until=YYYY-MM-DD        Exclusive upper bound
  --target=NAME             Filter by target host name
  --action=ACTION           Filter by action (command, run, apply, sql, ...)
  --run-id=ID               Filter by run id
  --execution-id=ID         Filter by execution id
  --error-kind=KIND         Filter by error kind
  --bypass-only             Only events that used a trust/safety bypass
  --to=FILE                 Export destination (audit export only)
  --json                    Emit the sshx.audit.v1 query document on stdout

  Empty results exit 0. Corrupt or partial records are reported as diagnostics
  and the valid records are retained.
...
MCP Server (stdio):
  ...
  Serves the execution contract to an MCP client over stdio. The server is
  spawned and owned by the client, lives for exactly one client session, and
  re-enters sshx as one-shot child processes per tool call: no listening socket,
  no resident state, no connection pool. Tools map 1:1 to CLI verbs and return
  the same versioned JSON, and password management is never exposed as a tool.
`
